...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function verifyContent(key, content) {
    ...
    if (!result) {
      ...
      Snackbar.show({text: wrongHashMessage, showAction: false});
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      hideButton.textContent = '　重新加密';  // 'Encrypt again';
      ...
      Snackbar.show({text: wrongPassMessage, showAction: false});
      console.log(e);
      return false;
    });
  ...
  function hbeLoader() {
    ...
    if (oldStorageData) {
      ...
      let id = setInterval(() => {
        if (window.Snackbar) {
          Snackbar.show({text: '&emsp;&emsp;这是一篇加密文章，内容可能涉及个人日常或者特殊技术分享，如果你确实想看，请与我联系，非亲友团勿扰。', showAction: false});
          clearInterval(id);
        }
      }, 1000);
    }
